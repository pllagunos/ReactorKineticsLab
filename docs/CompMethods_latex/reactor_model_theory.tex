...
lazily.  It loads the persisted clean solution immediately and derives the
fixed clean CE power-shape factor when a CE power mesh is available.  Each
response reads the current rod insertion from the point-kinetics simulation,
solves or reuses the corresponding rodded multigroup diffusion state, and
applies the fixed clean factor to the rodded diffusion power map.

If no valid SPH factor artifact is present, the endpoint still exposes the
uncorrected flux calculation and the post-solve power-map correction but marks
the result provisional and unqualified.  Rodded maps are also marked
...
\rule{0pt}{13pt}%
Multigroup diffusion solver & $G$-group 2D finite-volume with optional SPH & Solve the resolved-cell flux shape; display rodded power maps with a fixed clean CE power-shape correction \\
...
\begin{itemize}
  ...
  \item The older \textbf{one-group annular diffusion model} remains the
        simple spatial display model for the Core and Transient Diffusion
        pages.  Point-kinetics rod worth now comes from the OpenMC CE rod scan.
...
\texttt{theory/concentricModel.ipynb}, and the four-group rod-aware web page
