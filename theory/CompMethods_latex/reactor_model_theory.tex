...
web application exposes a four-group clean-core page backed by the same
adapter, factor artifact, operators, and persistent solution.
...
transport-calibrated rod-worth curve because there are no rodded MGXS,
rod-dependent SPH factors, or discontinuity factors in the present workflow.
Clean-core SPH factors, when available, do not qualify this rod perturbation.
It should not be frozen into the web application without a separate
validation study.
...
The web backend now exposes the four-group clean-core model through
\texttt{GET /api/multigroup-diffusion/state} and
\texttt{POST /api/multigroup-diffusion/recompute}.  The service is initialized
lazily.  It loads the persisted clean solution immediately, together with
matching SPH factors and corrected CSR operators when a valid artifact is
installed.  A fresh solve runs only after an explicit recompute request.

If no valid factor artifact is present, the endpoint still exposes the
uncorrected calculation but marks it provisional and unqualified.  The page
does not accept rod position and does not expose the exploratory equivalent-rod
model.  Immediate rod/reactivity response remains the responsibility of point
kinetics until rodded MGXS and rod-dependent diffusion corrections exist.
...
machinery are implemented and unit tested.  The presently stored four-group
export cannot yet produce a qualified factor set because it predates the raw
CE region/group flux and power-mesh payload.  Its CE eigenvalue standard
deviation is 59.34~pcm, while final qualification requires at most 15~pcm.
Therefore the online four-group result remains explicitly provisional until a
new high-statistics CE export is generated and the factor fit passes all flux,
power-shape, repeatability, and 50~pcm eigenvalue criteria.  Discontinuity
factors and all rodded corrections remain deferred.
...
\rule{0pt}{13pt}%
Multigroup diffusion solver & $G$-group 2D finite-volume with optional SPH & Solve and qualify the resolved-cell clean-core eigenvalue and flux/power shapes; retain the rod scan as offline exploration only \\
...
\begin{enumerate}
  ...
  \item parse the export through the adapter and build a boundary-fitted
        finite-volume mesh,
  \item qualify the production mesh, fit CE-referenced SPH factors, and
        persist corrected operators,
  \item solve and verify the clean-core multigroup diffusion eigenvalue problem
        through the CLI, notebook, or clean-core web page,
...
\begin{itemize}
  ...
  \item A \textbf{resolved multigroup diffusion solver} consumes those MGXS
        ...
        boundary-fitted cylindrical mesh, applies a persisted CE-referenced SPH
        factor set when available, and produces clean-core flux and power
        shapes.  The current stored export has not yet met the statistical and
        field-shape requirements for a qualified factor set.
...
\texttt{theory/concentricModel.ipynb}, and the four-group clean-core web page
are user-facing interfaces to the same multigroup pipeline.  The notebook
retains the uncalibrated equivalent-rod scan for offline exploration.  The
