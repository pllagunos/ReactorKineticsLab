...
\begin{quote}
The repository now contains \textbf{two parallel modeling tracks}: an
\textbf{OpenMC-informed resolved multigroup diffusion} pipeline that generates
transport-quality group constants from Monte Carlo and solves the eigenvalue
problem on the resolved cell geometry, and an \textbf{interactive one-group
web application} with a fast \textbf{point-kinetics surrogate} on the Overview
page and a \textbf{spatially resolved 2D diffusion transient} on the Transient
Diffusion page.  The multigroup pipeline is intended to recalibrate the
constants and rod-worth curves that the interactive models consume.
\end{quote}
...
\begin{enumerate}
  ...
  \item \textbf{Adapter and region mapping}
        ...
        convert constants to SI, and map each OpenMC cell to a
  ...
  \item \textbf{Persistent preparation cache}
        ...
        fingerprint the MGXS data, mesh settings, and solver configuration;
        build CSR matrices and a clean-core solution once, then persist
        everything to disk for fast reload.
\end{enumerate}

The user-facing entry point is the notebook
\texttt{theory/concentricModel.ipynb}, which selects an export directory,
configures mesh spacing and solver tolerances, triggers cache preparation, and
plots the resulting flux and rod-worth scan.
...
The transport correction applied (or not) to the within-group scatter term
is a separate concept and is deliberately excluded from the diffusion path.

Each export run produces:
\begin{itemize}
  \item \texttt{mgxs\_constants.json} --- nested group constants, delayed-neutron
        data, and metadata for every domain;
...
\begin{enumerate}
  ...
  \item \textbf{Scattering-matrix selection.}  Prefers
        ...
        Discards any Legendre moments above P0.
  \item \textbf{Unit conversion.}  Converts all macroscopic cross sections
        from cm$^{-1}$ to m$^{-1}$, diffusion coefficients from cm to m,
        inverse velocities from s/cm to s/m, and $\kappa\Sigma_f$ from
        eV/cm to J/m.
  \item \textbf{Inactive-group sanitization.}  Energy groups with zero
        ...
        replaced with the value from the nearest active group so the spatial
        operator remains well-posed.
\end{enumerate}
...
vectors for $D$, $\Sigma_a$, $\nu\Sigma_f$, $\chi$, and a $G\times G$ P0
scattering matrix $\Sigma_{s,g'\to g}$.  The radial mesh is fitted exactly
...
groups.  Within-group scattering is retained on the loss side through the
removal definition.

\subsubsection{Spatial discretization}

The mesh is a cell-centered finite-volume grid in cylindrical coordinates.
On each cell $(i,j)$ at radial position $r_i$ and axial position $z_j$, the
leakage operator is assembled as:

\begin{itemize}
  \item \textbf{Radial coupling:} face areas $r_{i\pm 1/2}\,\Delta z$ weighted
        by harmonically averaged diffusion coefficients at the interface.
  \item \textbf{Axial coupling:} face areas $r_i\,\Delta r$ weighted by
        harmonically averaged axial diffusion coefficients.
  \item \textbf{Diagonal loss:} cell volume $r_i\,\Delta r\,\Delta z$ times the
        local removal cross section (absorption plus total outscatter).
\end{itemize}

Harmonic averaging preserves current continuity at material discontinuities
and is the standard choice for cell-centered diffusion discretizations in
heterogeneous media.
...
\begin{enumerate}
  \item \textbf{Outer power iteration.}  Start from a positive flux guess
        ...
        \begin{enumerate}
          ...
          \item solve the coupled scattering system
                ...
                for all groups.
        \end{enumerate}
  \item \textbf{Inner scattering iteration.}  The coupled system is solved by
        Gauss--Seidel sweeps over groups with a scheduled source tolerance
        that tightens as the outer iteration progresses (from $10^{-2}$ in
        early iterations down to the user-specified \texttt{inner\_tol}).
  \item \textbf{Matrix-free GMRES acceleration.}  For strongly scattering
        cases, the inner iteration is wrapped in a matrix-free GMRES solver
        preconditioned by the same per-group SuperLU factors.  This
        accelerates convergence without ever assembling a monolithic
  ...
  \item \textbf{Flux normalization.}  The flux is normalized so the
        ...
        \]
        This is a physically meaningful power normalization: the flux
        magnitude directly scales with the reactor power level.
\end{enumerate}

Convergence is declared when $|k_{\mathrm{new}} - k_{\mathrm{old}}| <
\texttt{tol}$ and the volume-weighted balance residual falls below a
threshold.

\subsubsection{Rod-worth scan}

The equivalent-absorber rod model adds a scalar $\Delta\Sigma_a$ inside the
physical rod radius when the rod is inserted.  A rod scan sweeps the
insertion fraction $x \in [0, 1]$, solving the eigenvalue problem at each
position.  The scan may warm-start each solve with the previous converged
flux to reduce iteration counts.  The result is a reactivity curve
$\Delta\rho(x)$ that can be stored for later use by the web application.
...
every notebook restart or backend request.
...
\begin{itemize}
  ...
  \item the mesh-spacing configuration (target $\Delta r$ and $\Delta z$ per
        region),
  \item and the solver tolerances and algorithm version.
\end{itemize}
...
\begin{itemize}
  ...
  \item and the converged clean-core eigenvalue and flux field.
\end{itemize}
...
matrices and re-factorizes them in memory.  On a cache miss, the full
build-and-solve pipeline runs synchronously and the results are persisted.
...
\texttt{--prepare-cache} flag for use outside the notebook.

\subsection{Integration with the web application}

The multigroup diffusion pipeline currently operates as an offline
calibration tool.  The intended path to the web backend is:

\begin{enumerate}
  \item run the MGXS export once per group structure of interest,
  \item prepare the diffusion cache from the chosen export,
  \item extract the rod-worth curve $\Delta\rho(x)$ and the clean-core
        eigenvalue $k_{\mathrm{clean}}$,
  \item freeze those values into the backend calibration module, replacing
        or augmenting the current one-group effective constants,
  \item the web app then uses the multigroup-derived reactivity curve in its
        point-kinetics transient and the multigroup flux shape for spatial
        display.
\end{enumerate}

Because the cache stores per-group CSR matrices, a future backend could
also load a prepared cache and run rapid rod-position sweeps or
adjoint-weighted kinetics parameter calculations without re-running the
OpenMC transport step.

\subsection{Current status and verification}

The multigroup diffusion workflow is operational and verified across 1--70
energy groups on the resolved concentric geometry.  The workflow document
\texttt{theory/diffusion\_mgxs\_workflow.md} tracks per-group diffusion
results against both OpenMC continuous-energy and multigroup transport
reference $k_{\mathrm{eff}}$ values.  Key observations:

\begin{itemize}
  \item OpenMC multigroup transport converges to within $\sim\pm 50$~pcm of
        continuous-energy when both the geometry is resolved and P1/P3
        scattering is used.
  \item The uncorrected diffusion bias (diffusion $k_{\mathrm{eff}}$ minus
        ...
        for 8--70 groups, consistent with the expected systematic error from
        diffusion theory.
  \item The one-group diffusion result carries a $\sim+1800$~pcm bias,
        confirming that energy condensation and spatial homogenization are
        the dominant error sources in the older model.
\end{itemize}

SPH factors, discontinuity factors, and rodded MGXS are deferred until the
uncorrected diffusion baseline is stable and documented.
...
\rule{0pt}{13pt}%
Multigroup diffusion solver & $G$-group 2D finite-volume & Solve resolved-cell eigenvalue problem; produce rod-worth curve; anchor flux shape \\
Notebook estimate 1 & Homogeneous buckling & First-pass analytical check of scale and leakage trends \\
Notebook estimate 2 / Core page & Steady 2D one-group diffusion & Compute $k_{\mathrm{eff}}$, flux maps, and rod-worth calibration from effective constants \\
...
\begin{enumerate}
  ...
  \item parse the export through the adapter and build a boundary-fitted
        finite-volume mesh,
  \item solve the multigroup diffusion eigenvalue problem and scan the rod
        worth (via \texttt{concentricModel.ipynb}),
  \item freeze the resulting reactivity curve and clean-core constants into
        the backend calibration module,
  \item use the fast point model for dashboard-style operation on the
        Overview page,
...
The OpenMC-informed multigroup diffusion pipeline lifts the energy-group
limitation (arbitrary CASMO group structures up to 70 groups are supported)
and removes spatial homogenization by preserving every OpenMC cell as a
separate diffusion region.  However, it does not yet include:
\begin{itemize}
  \item SPH factors or discontinuity factors to correct the diffusion
        transport approximation,
  \item rodded MGXS (the rod scan still uses an equivalent absorber),
...
\begin{itemize}
  ...
  \item A \textbf{resolved multigroup diffusion solver} consumes those MGXS
        ...
        boundary-fitted cylindrical mesh, and produces rod-worth curves
        and flux shapes with substantially reduced energy-condensation
        error relative to the one-group model.
  \item The older \textbf{one-group annular diffusion model} remains the
        calibration workbench for the interactive web app, providing
        effective constants and rod-worth data for the point-kinetics and
        steady/transient diffusion pages.
  \item The \textbf{point-kinetics transient} on the Overview page gives
        fast control-response exploration in the browser.
  \item The \textbf{direct diffusion transient} on the Transient Diffusion
        page captures evolving spatial flux shapes during rod motion.
\end{itemize}

The notebook \texttt{theory/concentricModel.ipynb} is the user-facing
interface to the multigroup diffusion pipeline: select an MGXS export,
prepare the cache, inspect the flux, and scan the rod worth.  The
\texttt{diffusion\_mgxs\_workflow.md} document records the group-by-group
verification results and the scattering contract that ties the export
format to the solver expectations.
